...
function SubtasksPanel({ subtasks, onCreateSubtask, parentTask, currentUser }) {
  ...
  return (
    <div className="subtasks-panel bg-white border border-gray-200 rounded-lg overflow-hidden">
      {/* Enhanced Header with Progress */}
      <div 
        className="subtasks-header flex items-center justify-between p-4 border-b border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors bg-gradient-to-r from-blue-50 to-indigo-50"
      ...
      {/* Enhanced Content with Visual Hierarchy */}
      {!isCollapsed && (
          ...
          <div className="subtasks-list">
            <div className="subtask-indent">
              {filteredSubtasks.map((subtask, index) => (
                <div 
                  key={subtask.id} 
                  className={`subtask-item transition-all duration-200 ${
                    selectedSubtask?.id === subtask.id ? 'selected' : ''
                  }`}
                    ...
                    <div className="subtask-content">
                      ...
                      <div className="subtask-meta">
                        <div className="subtask-assignee">
                          <div className="subtask-avatar">
                            {subtask.assignee?.charAt(0) || 'U'}
                          </div>
                          <span className="subtask-assignee-name">{subtask.assignee}</span>
                        </div>
                        
                        <div className={`subtask-due-date ${isOverdue(subtask.dueDate) && subtask.status !== 'completed' ? 'overdue' : ''}`}>
                          Due: {subtask.dueDate}
                          {isOverdue(subtask.dueDate) && subtask.status !== 'completed' && (
                            <span className="ml-1">⚠️</span>
                          )}
                        </div>
                        
                        <div className={`subtask-priority ${subtask.priority}`}>
                          {subtask.priority}
                        </div>
                      </div>
                    </div>
            ...
            {/* Sticky Add Button */}
            {!showInlineAdd && filteredSubtasks.length > 0 && (
              <div className="sticky-add-subtask">
                <button 
