...
export default function ApprovalTaskDetailModal({ task, onClose, currentUser, onApproval }) {
  ...
  const currentUserApprover = task.approvers?.find(a => a.id === currentUser.id);
  const canApprove = currentUserApprover && currentUserApprover.status === 'pending';
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 font-medium capitalize">{task.approvalMode}</span>
              </div>
            </div>
          </div>
              ...
              {task.approvers?.map((approver) => (
                <div key={approver.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    ...
                    {approver.comment && (
                      <p className="text-xs text-gray-500 mt-1">{approver.comment}</p>
                    )}
                  </div>
                </div>
              ))}
