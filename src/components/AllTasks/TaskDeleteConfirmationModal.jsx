...
export default function TaskDeleteConfirmationModal({ task, options, onConfirm, onCancel, currentUser }) {
  ...
  const isReferencedInDependencies = task.dependencies && task.dependencies.length > 0;
        ...
        <div className="p-6">
          ...
          <div className="mb-6">
            <p className="text-gray-900 mb-2">
              Are you sure you want to delete {isSubtask ? 'this sub-task' : 'this task'}:
            </p>
                ...
                <div className="flex items-start">
                  ...
                  <div className="ml-3">
                    <h4 className="text-sm font-medium text-orange-800">
                      This task has {task.subtasks.length} sub-task{task.subtasks.length !== 1 ? 's' : ''}
                    </h4>
                ...
                <div className="flex items-start">
                  ...
                  <div className="ml-3">
                    ...
                    <p className="text-sm text-purple-700 mt-1">
                      This task is referenced by other tasks. Deleting it may affect task relationships.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-start">
                  ...
                  <div className="ml-3">
                    ...
                    <p className="text-sm text-yellow-700 mt-1">
                      This task was created by {task.createdBy}. You can delete it because you are {task.assigneeId === currentUser.id ? 'assigned to it' : 'an administrator'}.
                    </p>
                  </div>
                </div>
              </div>
          ...
          {(hasSubtasks || hasAttachments || hasLinkedItems || hasForms) && (
            <div className="mb-6 space-y-3">
              <p className="text-sm font-medium text-gray-700">Deletion Options:</p>
              ...
              {hasSubtasks && (
                <label className="flex items-center">
                  ...
                  <span className="ml-3 text-sm text-gray-700">
                    Also delete all {task.subtasks.length} sub-task{task.subtasks.length !== 1 ? 's' : ''}
                  </span>
                </label>
              )}
              ...
              {hasLinkedItems && (
                <label className="flex items-center">
                  ...
                  />
                  <span className="ml-3 text-sm text-gray-700">
                    Also delete {task.linkedItems.length} linked item{task.linkedItems.length !== 1 ? 's' : ''}
                  </span>
                </label>
              )}
            </div>
          ...
          <div className="mb-6">
            <label className="flex items-start">
              <input
                ...
                onChange={(e) => handleOptionChange('confirmed', e.target.checked)}
              ...
              <span className="ml-3 text-sm text-gray-900 font-medium">
                I understand this action is <span className="text-red-600">irreversible</span> and will permanently delete this {isSubtask ? 'sub-task' : 'task'} and all selected related data.
              </span>
            </label>
          </div>
